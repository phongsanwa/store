...
@section('scripts')
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $('body').on('click', '#show-role', function () {
                var userURL = $(this).data('url');
                $.get(userURL, function (data) {
                    let permission = data.namePermission;
                    // console.log(permission);
                    $('#roleShowModal').modal('show');
                    $('.name-role').text(data.nameRole.name);
                    $.each(permission, function(i, item) {
                            $('.table-striped tbody').append(
                                '<tr>'+
                                '<td>' + permission[i].name + '</td>'+
                                '<td>' + permission[i].guard_name + '</td>' +
                                '</tr>'
                            );
                    });
                })
            ...
            <div class="row">
                <div class="col-12">
                    <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                        <h4 class="mb-sm-0 font-size-18">Danh mục vai trò</h4>

                        <div class="page-title-right">
                            <ol class="breadcrumb m-0">
                                <li class="breadcrumb-item"><a href="javascript: void(0);"><i class="fa fa-home-alt mr-2"></i></a></li>
                                <li class="breadcrumb-item active">Danh mục vai trò</li>
                            </ol>
                        </div>
                    ...
                    <div class="card">
                        <div class="card-body">
                            <div class="row mb-2">
                                ...
                                <div class="col-sm-4">
                                    <div class="text-sm-end">
                                        <a href="{{ route('roles.create') }}" class="text-white"><button type="button" class="btn rounded btn-success btn-rounded waves-effect waves-light mb-2 me-2"><i class="mdi mdi-plus me-1"></i>Thêm mới </button></a>
                                    </div>
                                </div>
                                <!-- end col-->
                            </div>
                                            ...
                                            <td class="setting-custom">
                                                <div class="d-block text-center dropdown">
                                                    ...
                                                    <ul class="dropdown-menu">
                                                        <li><a class="dropdown-item" href="javascript:void(0)" id="show-role" data-url="{{ route('roles.show', $p->id) }}" ><i class="fal fa-eye"></i>Xem vai trò</a></li>
                                                        <li><a class="dropdown-item" href="{{ route('roles.edit',$p->id) }}"><i class="fal fa-pencil"></i>Sửa sản phẩm</a></li>
                            ...
                            <div class="paginator-content">
                                {{$roles->links('back-end.layouts.pagination')}}
                            </div>
                        </div>
                    </div>
                </div>
...
{{--Modal show Role for each of all--}}
    <div class="modal fade" id="roleShowModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Tên vai trò: <span class="name-role"></span></h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
